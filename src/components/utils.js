async function getToken () {
    // import.meta.env.VITE_API_TOKEN_URL
    const resp = await fetch(`https://accaunts.spotify.com/api/token`, {
        method: "POST",
        headers: {
            'Content-type' : 'application/x-www-form-urlencoded',
            'Authorization': `Basic ${btoa(import.meta.env.VITE_CLIENT_ID + ':' + import.meta.env.VITE_SECRET_KEY_env)}`
        },
        body: 'grant_type=client_credentials'
    })
    const auth = await resp.json();
    const expireDate = getOneHourLater();
    localStorage.setItem('expireData', expireDate)
    localStorage.setItem('token', auth.access_token)
    return {
        date: expireDate,
        token: auth.access_token
    }
}
function getOneHourLater () {
   let currentDate = new Date();

   let oneHourLater = new Date(currentDate.getTime() + 3600000)

   let day = String(oneHourLater.getDate()).padStart(2, '0')
   let month = String(oneHourLater.getDate() +1).padStart(2, '0')
   let year =  oneHourLater.getFullYear();

   let hours = String(oneHourLater.getHours()).padStart(2, '0');
   let minutes = String(oneHourLater.getMinutes()).padStart(2, '0');

   let formattedDataTime = `${day}.${month}.${year} ${hours}:${minutes}`
   
   return formattedDataTime  
}

console.log(getOneHourLater());


export {getToken}